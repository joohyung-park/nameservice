...
func (k Keeper) GetWhoisCount(ctx sdkTypes.Context) int64 {
...
func (k Keeper) SetWhoisCount(ctx sdkTypes.Context, count int64) {
	store := ctx.KVStore(k.storeKey)
	byteKey := []byte(types.WhoisCountPrefix)
...
// func (k Keeper) CreateWhois(ctx sdkTypes.Context, whois types.Whois) {
// 	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetWhois(ctx sdkTypes.Context, key string) (types.Whois, error) {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) SetWhois(ctx sdkTypes.Context, name string, whois types.Whois) {
...
func (k Keeper) DeleteWhois(ctx sdkTypes.Context, key string) {
	store := ctx.KVStore(k.storeKey)
...
func listWhois(ctx sdkTypes.Context, k Keeper) ([]byte, error) {
	var whoisList []types.Whois
	store := ctx.KVStore(k.storeKey)
	iterator := sdkTypes.KVStorePrefixIterator(store, []byte(types.WhoisPrefix))
...
func getWhois(ctx sdkTypes.Context, path []string, k Keeper) (res []byte, sdkError error) {
	key := path[0]
...
func resolveName(ctx sdkTypes.Context, path []string, keeper Keeper) ([]byte, error) {
...
func (k Keeper) GetCreator(ctx sdkTypes.Context, key string) sdkTypes.AccAddress {
	whois, _ := k.GetWhois(ctx, key)
	return whois.Creator
}

// Check if the key exists in the store
func (k Keeper) Exists(ctx sdkTypes.Context, key string) bool {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) HasCreator(ctx sdkTypes.Context, name string) bool {
	whois, _ := k.GetWhois(ctx, name)
	return !whois.Creator.Empty()
}

// SetOwner - sets the current owner of a name
func (k Keeper) SetCreator(ctx sdkTypes.Context, name string, creator sdkTypes.AccAddress) {
	whois, _ := k.GetWhois(ctx, name)
...
func (k Keeper) IsNamePresent(ctx sdkTypes.Context, name string) bool {
	store := ctx.KVStore(k.storeKey)
...
func (k Keeper) GetNamesIterator(ctx sdkTypes.Context) sdkTypes.Iterator {
	store := ctx.KVStore(k.storeKey)
	return sdkTypes.KVStorePrefixIterator(store, []byte(types.WhoisPrefix))
}

// Get creator of the item
func (k Keeper) GetWhoisOwner(ctx sdkTypes.Context, key string) sdkTypes.AccAddress {
	whois, err := k.GetWhois(ctx, key)
	if err != nil {
		return nil
	}
...
// Check if the key exists in the store
func (k Keeper) WhoisExists(ctx sdkTypes.Context, key string) bool {
	store := ctx.KVStore(k.storeKey)
	return store.Has([]byte(types.WhoisPrefix + key))
}
