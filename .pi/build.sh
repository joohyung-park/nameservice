...
# docker buildx build --tag nameservice --file .pi/Dockerfile --platform linux/arm64 --cache-from nameservice:cache --cache-to nameservice:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar nameservice
...
sudo bash -c "echo nameservice > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/nameservice.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/nameservice.img
